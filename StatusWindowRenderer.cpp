...
void StatusWindowRenderer::draw()
{
    ...
    display.drawText(posX + 10, posY + 20,
        "‚È‚Ü‚¦: " + target->name, col);

    display.drawText(posX + 10, posY + 40,
        "HP: " + std::to_string(target->hp) +
        " / " + std::to_string(target->maxHp), col);

    display.drawText(posX + 10, posY + 60,
        "MP: " + std::to_string(target->mp) +
        " / " + std::to_string(target->maxMp), col);

    display.drawText(posX + 10, posY + 80,
        "‚±‚¤‚°‚«—Í: " + std::to_string(target->attackPower), col);

    display.drawText(posX + 10, posY + 100,
        "‚Ú‚¤‚¬‚å—Í: " + std::to_string(target->defensePower), col);

    display.drawText(posX + 10, posY + 120,
        "‚·‚Î‚â‚³: " + std::to_string(target->speed), col);

    display.drawText(posX + 10, posY + 140,
        "Lv: " + std::to_string(target->level), col);

    display.drawText(posX + 10, posY + 160,
        "‚¯‚¢‚¯‚ñ‚¿: " + std::to_string(target->gainedExp), col);
}
